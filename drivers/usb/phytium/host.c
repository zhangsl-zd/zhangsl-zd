...
static void hostOtgIrq(struct HOST_CTRL *priv)
{
	...
	if (otgirq & OTGIRQ_CONIRQ) {
		if (priv->otgState == HOST_OTG_STATE_A_HOST ||
			...
			if (otgirq & OTGIRQ_HOSTDISCON)
				disconnectHostDetect(priv);
			else if (otgstate == HOST_OTG_STATE_A_WAIT_VFALL ||
	...
	phytium_write8(&priv->regs->otgirq, OTGIRQ_IDCHANGEIRQ |
			OTGIRQ_HOSTDISCON | OTGIRQ_SRPDETIRQ);
}
...
static int32_t hostEpXIrq(struct HOST_CTRL *priv, uint8_t hwEpNum, uint8_t isIn)
{
	struct HostEp *hwEp;
	struct HOST_EP *hostEp;
	struct HOST_EP_PRIV *usbHEpPriv;
...
static void host_CallbackTransfer(void *priv, uint8_t epNum, uint8_t isDirTx)
{
	...
	if (!epNum)
		...
		hostEpXIrq(host_priv, epNum, !isDirTx);
}
		...
		struct HOST_CALLBACKS *callbacks, struct device *pdev, bool isVhubHost)
{
	...
	memset(priv, 0, sizeof(struct HOST_CTRL));
	...
	for (index = 0; index < MAX_SUPPORTED_DEVICES; index++)
		priv->host_devices_table[index] = NULL;
...
void hostStart(struct HOST_CTRL *priv)
{
	...
	phytium_write8(&priv->regs->usbien, USBIR_URES | USBIR_SUSP);
retry:
	otgstate = phytium_read8(&priv->regs->otgstate);
	switch (otgstate) {
	...
	phytium_write8(&priv->regs->otgien, OTGIRQ_CONIRQ |
			...
			OTGIRQ_BSE0SRPIRQ | OTGIRQ_HOSTDISCON);
}
...
void hostIsr(struct HOST_CTRL *priv)
{
	...
	if (usbirq & USBIR_SUSP) {
		...
		phytium_write8(&priv->regs->clkgate, 0x0);
		...
		struct HOST_REQ *req, struct HOST_EP *usbEp)
{
	...
	if (hostEp->isInEp) {
		...
		phytium_write16(&priv->regs->rxerrien, rxerrien);
		//phytium_write8(&priv->regs->endprst, ENDPRST_IO_TX | 0);
		...
		phytium_write16(&priv->regs->txerrien, txerrien);
		//phytium_write8(&priv->regs->endprst, hostEp->hwEpNum);
		phytium_write8(&priv->regs->endprst, ENDPRST_FIFORST | hostEp->hwEpNum);
	}

	status = priv->dmaDrv->dma_channelAbort(priv->dmaController, hostEp->channel);
...
int32_t hostReqDequeue(struct HOST_CTRL *priv, struct HOST_REQ *req, uint32_t status)
{
	...
	if (!usbEpPriv->currentHwEp || req->list.prev != &usbEp->reqList) {
		...
		if (list_empty(&usbEp->reqList)) {
			if (usbEpPriv->currentHwEp && usbEpPriv->currentHwEp->channel) {
				if (priv->dmaDrv->dma_getChannelStatus(priv->dmaController,
						usbEpPriv->currentHwEp->channel) >= DMA_STATUS_BUSY)
					priv->dmaDrv->dma_controllerReset(priv->dmaController);
			}
...
static int phytium_host_set_default_cfg(struct phytium_cusb *config)
{
	...
	for (index = 0; index < HOST_EP_NUM; index++) {
		if (index == 0) {
			...
			config->host_cfg.epIN[index].bufferingValue = 4;
			config->host_cfg.epIN[index].maxPacketSize = 1024;
			config->host_cfg.epIN[index].startBuf = 64 + 4096 * (index - 1);

			config->host_cfg.epOUT[index].bufferingValue = 4;
			config->host_cfg.epOUT[index].maxPacketSize = 1024;
			config->host_cfg.epOUT[index].startBuf = 64 + 4096 * (index - 1);
		}
	}
...
static irqreturn_t host_usb_irq(int irq, void *dev_id)
{
	unsigned long flags;
...
int phytium_host_init(struct phytium_cusb *config)
{
	...
	*config->hcd->hcd_priv = (unsigned long)config;
	...
	if (config->irq > 0) {
		ret = devm_request_irq(config->dev, config->irq,
				host_usb_irq, IRQF_SHARED, DRV_NAME, config);
		if (ret != 0) {
			dev_err(config->dev, "reqeust irq %d err %d\n", config->irq, ret);
			ret = -ENODEV;
			goto err_irq;
		}
	}
