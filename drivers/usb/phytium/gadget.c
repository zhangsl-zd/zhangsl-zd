...
void gadget_CallbackTransfer(void *priv, uint8_t epNum, uint8_t epDir)
{
	if (!epNum)
		gadgetEp0Irq(priv);
	else
		...
		struct GADGET_CALLBACKS *callbacks, struct device *pdev)
{
	...
	memset(priv, 0, sizeof(struct GADGET_CTRL));
...
static irqreturn_t gadget_irq(int irq, void *dev_id)
{
	unsigned long flags;
	struct phytium_cusb *config;
	...
	config = (struct phytium_cusb *)dev_id;
...
int phytium_gadget_init(struct phytium_cusb *config)
{
	...
	if (config->irq > 0) {
		ret = devm_request_irq(config->dev, config->irq,
				gadget_irq, IRQF_SHARED, DRV_NAME, config);
		if (ret != 0) {
