...
static int phytium_qspi_read_flash_sfdp(struct phytium_qspi *qspi,
	struct phytium_qspi_flash *flash, u8 opcode, loff_t from, u8 *buf, int len)
{
	unsigned long iflags;
	u32 cmd = 0;
				 ...
				 u_char *buf)
{
	...
	case 0x5A:
		cmd &= ~QSPI_RD_CFG_RD_ADDR_SEL_MASK;
		phytium_qspi_read_flash_sfdp(qspi, flash, nor->read_opcode, from, buf, len);
		return 0;
		break;
	default:
		break;
				    ...
				    struct device_node *np)
{
	...
	of_property_read_u32(np, "spi-clk-div", &clk_div);
	if (!clk_div)
	...
	of_property_read_u32(np, "spi-rx-bus-width", &width);
	if (!width)
		width = 1;
