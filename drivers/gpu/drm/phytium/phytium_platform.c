...
phytium_platform_private_init(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "missing pipe_mask property\n");
		goto failed;
	}

	ret = of_property_read_u8(node, "edp_mask", &priv->info.edp_mask);
	if (ret < 0) {
		dev_err(&pdev->dev, "missing edp_mask property\n");
		goto failed;
	}
...
static const struct of_device_id phytium_platform_match[] = {
	{
...
struct platform_driver phytium_platform_driver = {
	.driver = {
		...
		.of_match_table = of_match_ptr(phytium_platform_match),
	},
