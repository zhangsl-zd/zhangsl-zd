...
int phytium_dp_init(struct drm_device *dev, int port)
{
	...
	if (IS_X100(priv)) {
		...
		priv->dp_reg_base[port] = X100_DP_BASE(port);
		priv->phy_access_base[port] = X100_PHY_ACCESS_BASE(port);
	} else if (IS_E2000(priv)) {
		e2000_dp_func_register(phytium_dp);
		priv->dp_reg_base[port] = E2000_DP_BASE(port);
		priv->phy_access_base[port] = E2000_PHY_ACCESS_BASE(port);
	}
