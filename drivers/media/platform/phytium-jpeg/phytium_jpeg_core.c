...
static void phytium_jpeg_init_regs(struct phytium_jpeg_dev *jpeg_dev)
{
	...
	/* reset case, wait interrupt 0.5 seconds */
...
static inline void phytium_jpeg_enable_source_detecting(struct phytium_jpeg_dev *jpeg_dev)
{
	 ...
	 */
	phytium_jpeg_update(jpeg_dev, INT_STATUS_CTRL_REG, 0, DETECT_RESOLUTON_CHANGE_EN);
...
static void phytium_jpeg_get_resolution(struct phytium_jpeg_dev *jpeg_dev)
{
	...
	/* Enable detecting the changing of the input video */
	set_bit(VIDEO_RES_DETECT, &jpeg_dev->status);
	phytium_jpeg_enable_source_detecting(jpeg_dev);
	ret = wait_event_interruptible_timeout(jpeg_dev->wait,
			res_check(jpeg_dev), SOURCE_RESOLUTION_DETECT_TIMEOUT);
	/*
	 * Timeout,interrupt isn't triggered,
	 * derive that resolution isn't changed
	 */
	if (ret == 0)
		dev_info(jpeg_dev->dev, "Timeout, source resolution doesn't change\n");
					    ...
					    struct v4l2_frmivalenum *fival)
{
	...
	if (fival->width != jpeg_dev->detected_timings.width ||
		...
		dev_err(jpeg_dev->dev, "enum frame interval isn't same with the detected_timings.\n");
		return -EINVAL;
	}
					 ...
					 struct v4l2_dv_timings *timings)
{
	...
	/*
	 * TODO: This blocks only if the driver is currently in the process of
...
/* TODO: wait to implement the complete function */
...
static irqreturn_t phytium_jpeg_irq(int irq, void *arg)
{
	...
	u32 status = phytium_jpeg_read(jpeg_dev, INT_STATUS_CTRL_REG);
	struct phytium_jpeg_buffer *buf;
	u32 frame_size;

	dev_info(jpeg_dev->dev, "read interrupt status 0x%x\n", status);
	/* The resolution of the source video is changed,
	 * reset the engine  and re-initialize
	 */
	dev_info(jpeg_dev->dev, "jpeg device status 0x%x\n",jpeg_dev->status);
	if (status & INT_VIDEO_FORMAT_CHANGE) {
		if (test_bit(VIDEO_RES_DETECT, &jpeg_dev->status)) {
			phytium_jpeg_update(jpeg_dev, INT_STATUS_CTRL_REG,
					    DETECT_RESOLUTON_CHANGE_EN, 0);
			phytium_jpeg_write(jpeg_dev, INT_STATUS_CTRL_REG,
					   INT_VIDEO_FORMAT_CHANGE);
			status &= ~INT_VIDEO_FORMAT_CHANGE;
			set_bit(VIDEO_MODE_DETECT_DONE, &jpeg_dev->status);
			clear_bit(VIDEO_RES_DETECT, &jpeg_dev->status);
			wake_up_interruptible_all(&jpeg_dev->wait);
		} else {
	...
	status &= ~STS_JPEG_COMP_BUF_NO;
	return status != 0 ? IRQ_NONE : IRQ_HANDLED;
}
...
static int phytium_jpeg_init(struct phytium_jpeg_dev *jpeg_dev)
{
	...
	/* TODO: CMA memory for JPEG device */
...
static int phytium_jpeg_setup_video(struct phytium_jpeg_dev *jpeg_dev)
{
	...
	/* TODO: register how many v4l2 controls to a handler */
	...
	vdev->vfl_type = VFL_TYPE_GRABBER; /* newer kernel version using VFL_TYPE_VIDEO */
...
static const struct phytium_jpeg_config phytium_e2000_config = {
